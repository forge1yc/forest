...
type JobSnapshotProcessor struct {
	...
	jobs                map[string]Job // job 是接口，实现这个接口的都可以传进去 // 这里需要改成一个处理器，从grindstone 修改过来
	lk                  *sync.RWMutex
}

// new a job snapshot processor
func NewJobSnapshotProcessor(group, ip string, etcd *ectd.Etcd) *JobSnapshotProcessor {
	...
	processor.snapshotPath = fmt.Sprintf(global.JobSnapshotPrefix, group, ip)
	processor.snapshotExecutePath = fmt.Sprintf(global.JobExecuteSnapshotPrefix, group, ip)
...
func (processor *JobSnapshotProcessor) handleSnapshot(snapshot *JobSnapshot) {
	...
	executeSnapshot := &JobExecuteSnapshot{
		...
		Status:     global.JobExecuteDoingStatus,
	...
	if target == "" { // 所以这个target只是用来识别的吗
		...
		executeSnapshot.Status = global.JobExecuteUkonwStatus
		return
	}
	...
	if executeSnapshot.Status != global.JobExecuteDoingStatus { // 多线程的原因，别人会改这里吗，所以加这一个判断，因为上面接手处理的时候就是doing初始化的
		return
	}
	...
	if result, err = executeSnapshot.execute(); err != nil { // 直接执行，完了反馈结果
		executeSnapshot.Status = global.JobExecuteErrorStatus
	}
	after := time.Now()
	executeSnapshot.Status = global.JobExecuteSuccessStatus
	...
	executeSnapshot.FinishTime = after.Format("2006-01-02 15:04:05") // 创建时间，执行时间，好多东西可以添加进来
