...
// new a client
func NewForestClient(group, ip string, etcd *ectd.Etcd) *ForestClient {
...
func (client *ForestClient) Bootstrap() (err error) {
	...
	client.clientPath = fmt.Sprintf(global.ClientPathPrefix, client.group, client.ip)
	client.snapshotPath = fmt.Sprintf(global.JobSnapshotPrefix, client.group, client.ip)
	...
	client.running = true
	<-client.quit // 除非主动发送，不然一直阻塞在这里，因为没有人call Stop()
...
RETRY:
	...
	if client.state == global.RegistryState {
	...
	client.state = global.RegistryState
	...
	case <-client.txResponse.StateChan: // 这里会阻塞 因为只有失败的时候会传回来
		client.state = global.URegistryState
		log.Printf("the forest client fail registry to----->:%s", client.clientPath)
...
func (client *ForestClient) lookup() {
	...
	for {
		...
		if client.state == global.URegistryState {
			time.Sleep(time.Second * 3)
			continue
		}
		...
		for i := 0; i < len(values); i++ {
			...
			if client.state == global.URegistryState { // 这里还要检测，因为有可能挂掉了
				time.Sleep(time.Second * 3)
				break
			}
